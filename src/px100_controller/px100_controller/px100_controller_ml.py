...
class ArmController(InterbotixManipulatorXS):
    ...
    def __init__(self, pargs, args=None):
        ...
        # self.model = load(
        #     "/home/xavier/projects/datasets/mlp_regressor_weights.joblib")
        self.model.load_state_dict(torch.load(
            '../ik_solver_with_ml/better_model.state_dict', weights_only=True))
        self.model.eval()
    ...
    def move_end_effector(self) -> None:
        ...
        with torch.no_grad():
            input_tensor = torch.tensor([
                desired_T_virtual_to_body[0, 3],
                desired_T_virtual_to_body[2, 3],
                desired_rpy[1]
            ], dtype=torch.float32).unsqueeze(0)
            ml_joints = self.model(input_tensor)
            waist_cmd = self.arm.get_single_joint_command('waist')
            self.log_info(
                f'{ml_joints} vs. {self.move_end_effector_wrt_virtual_frame(desired_T_virtual_to_body, T_virtual_to_body, desired_rpy, rpy)}')
    ...
    def move_end_effector_wrt_virtual_frame(self, desired_T_virtual_to_body,
        ...
        if move_end_effector:
            ...
            mr_joint_cmds, success = self.arm.set_ee_pose_matrix(
                T_sd=T_sd,
    ...
    def get_inference(self, x, y, z, pitch, yaw):
        arr = np.array([x, y, z, pitch, yaw]).reshape(1, -1)
