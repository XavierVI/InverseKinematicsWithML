...
class ArmController(InterbotixManipulatorXS):
    ...
    def move_end_effector_wrt_virtual_frame(self, desired_T_virtual_to_body,
        ...
        if move_end_effector:
            ...
            L2 = 1.0
            Lm = 0.35
            Lr = math.sqrt(L2*L2 + Lm*Lm)
            L3 = 1.0
            L4 = 0.8605
            a_vec = T_sd[:3, 0]
            xw, yw, zw = T_sd[:3, 3] - L4*a_vec
            r = math.sqrt(xw**2 + yw**2)
            h = zw - L1
            c = math.sqrt(r**2 + h**2)
            beta = math.atan2(Lm, L2)
            psi = (math.pi / 2) - beta
            phi_num = c*c - L3*L3 - Lr*Lr
            phi_dem = -2*Lr*L3
            if abs(phi_num / phi_dem) > 1:
                self.log_info(f'Failed')
                return

            phi = math.acos(phi_num / phi_dem)
            gamma = math.atan2(h, r)
            alpha_num = L3*L3 - Lr*Lr - c*c
            alpha_dem = -2*Lr*c
            if abs(alpha_num / alpha_dem) > 1:
                self.log_info(f'Failed')
                return
            alpha = math.acos(alpha_num / alpha_dem)
            waist = math.atan2(T_sd[1, 3], T_sd[0, 3])
            shoulder = (math.pi / 2) - beta - alpha - gamma
            elbow = math.pi - psi - phi
            theta_a = math.atan2(
                math.sqrt(a_vec[0]**2 + a_vec[1]**2), a_vec[2])
            wrist = theta_a - shoulder - elbow
            error = np.array(self.arm.get_joint_commands()) - \
                [waist, shoulder, elbow, wrist]
            print(f'Error: {error}')
            joints = [waist, shoulder, elbow, wrist]
            self.arm.set_joint_positions(joints)

            # self.log_info(f'Correct joint angles:\n {correct_joint_cmd}')
            # self.log_info(f'Joint angles from analytical IK solution:\n {joints}')
